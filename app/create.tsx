...
export default function CreateWalletScreen() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (wordCount !== 12) throw new Error(`Expected 12 words, got ${wordCount}.`);
  ...
  const handleBackupConfirm = useCallback(() => {
    if (!backedUp) { showAlert('Backup Required', 'Please confirm you have written down your seed phrase.'); return; }
    setPin(''); setConfirmPin(''); setPinError(''); setStep('pin');
  }, [backedUp, showAlert]);
  ...
  const handleCreateWallet = useCallback(async () => {
    ...
    if (trimmedPin !== trimmedConfirm) { setPinError('PINs do not match.'); return; }
    if (!mnemonic) { showAlert('Error', 'No seed phrase found. Please go back.'); return; }
    if (loading) return;
    setPinError('');
    setLoading(true);
    try {
      ...
      showAlert('Error', 'Could not create wallet. ' + String(err));
    } finally {
      setLoading(false);
    }
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
      <View style={styles.container}>
        {/* Gradient Header */}
        <LinearGradient
          colors={['#7C3AED', '#4D49FC', '#0EA5E9']}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
          style={[styles.header, { paddingTop: insets.top + (Platform.OS === 'web' ? 60 : 16) }]}
        >
          <Pressable
            onPress={() => {
              ...
              return (
                <React.Fragment key={s}>
                  <View style={[styles.stepDot, isActive && styles.stepDotActive, isDone && styles.stepDotDone]}>
                    {isDone
                      ? <MaterialIcons name="check" size={11} color="#4D49FC" />
                      : <Text style={[styles.stepDotText, isActive && styles.stepDotTextActive]}>{idx + 1}</Text>}
                  </View>
                  {idx < STEPS.length - 1 && (
          ...
          <Text style={styles.stepLabel}>{stepLabels[currentIdx]}</Text>
        </LinearGradient>
        ...
        >
            ...
            <View style={styles.section}>
              <View style={styles.infoCard}>
                ...
                <Text style={styles.infoText}>
                  A 12-word seed phrase will be generated on your device. This phrase is the ONLY way to recover your wallet.
                </Text>
              ...
              <Pressable
                ...
                disabled={loading}
                style={({ pressed }) => [styles.primaryBtn, pressed && styles.pressed, loading && { opacity: 0.7 }]}
              >
                <Text style={styles.primaryBtnText}>{loading ? 'Generating…' : 'Generate Seed Phrase'}</Text>
              </Pressable>
            </View>
          )}
          ...
          {step === 'backup' && words.length === 12 && (
            <View style={styles.section}>
              ...
              <Text style={styles.stepSubtitle}>Write down all 12 words in order. You will need them to restore your wallet.</Text>
              ...
              <View style={styles.seedCard}>
                ...
                <Pressable onPress={() => setObscured(!obscured)} style={styles.revealBtn}>
                  <MaterialIcons name={obscured ? 'visibility' : 'visibility-off'} size={16} color={Colors.primary} />
              ...
              <View style={styles.warnBanner}>
                ...
                <Text style={styles.warnText}>Never screenshot or store this digitally. Anyone with these words controls your funds.</Text>
              </View>
              ...
              <Pressable onPress={() => setBackedUp(!backedUp)} style={styles.checkRow} hitSlop={4}>
                <View style={[styles.checkbox, backedUp && styles.checkboxChecked]}>
                  {backedUp && <MaterialIcons name="check" size={13} color="#fff" />}
                </View>
                <Text style={styles.checkLabel}>I have written down my seed phrase and stored it securely offline</Text>
              </Pressable>

              <Pressable
                onPress={handleBackupConfirm}
                disabled={!backedUp}
                style={({ pressed }) => [styles.primaryBtn, !backedUp && { opacity: 0.45 }, pressed && styles.pressed]}
              >
          ...
          {step === 'pin' && (
            <View style={styles.section}>
              <View style={styles.pinHeaderCard}>
                ...
                <Text style={styles.stepSubtitle}>This PIN will be used to unlock your wallet every time you open the app.</Text>
              </View>
              ...
              <View style={styles.pinNote}>
                ...
                <Text style={styles.pinNoteText}>Store your PIN somewhere safe. It cannot be recovered but you can restore your wallet with your seed phrase.</Text>
              </View>

              <Pressable
                onPress={handleCreateWallet}
                disabled={loading || pin.length < 6 || confirmPin.length < 6}
                style={({ pressed }) => [styles.primaryBtn, (loading || pin.length < 6 || confirmPin.length < 6) && { opacity: 0.45 }, pressed && styles.pressed]}
              >
                <Text style={styles.primaryBtnText}>{loading ? 'Creating Wallet…' : 'Create Wallet'}</Text>
              </Pressable>
            </View>
          )}
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  stepRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 4 },
  stepDot: {
  ...
  stepDotText: { fontSize: 11, fontWeight: '600', color: 'rgba(255,255,255,0.7)' },
  stepDotTextActive: { color: '#4D49FC' },
  stepLine: { flex: 1, height: 2, backgroundColor: 'rgba(255,255,255,0.25)', borderRadius: 1 },
  stepLineDone: { backgroundColor: '#fff' },
  stepLabel: { fontSize: 12, color: 'rgba(255,255,255,0.75)', fontWeight: '500', letterSpacing: 0.4 },
  ...
  revealText: { fontSize: 13, color: Colors.primary },
  warnBanner: {
  ...
  primaryBtn: {
    height: 54, backgroundColor: Colors.primary, borderRadius: Radii.md,
    alignItems: 'center', justifyContent: 'center',
